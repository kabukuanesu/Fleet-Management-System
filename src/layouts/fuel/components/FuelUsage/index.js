import * as React from "react";
import { useState, useEffect } from "react";
import { DataGrid } from "@mui/x-data-grid";
import Paper from "@mui/material/Paper";
import PopFuel from "./data";
import Dialog from "@mui/material/Dialog";
import DialogTitle from "@mui/material/DialogTitle";
import DialogContent from "@mui/material/DialogContent";
import IconButton from "@mui/material/IconButton";
import CloseIcon from "@mui/icons-material/Close";

const columns = [
  { field: "vehicleRegistrationId", headerName: "Vehicle", width: 130 },
  { field: "vehicleFuelType", headerName: "Type", width: 130 },
  { field: "vehicleFuelQuantity", headerName: "Quantity", width: 130 },
  { field: "vehicleFuelPrice", headerName: "Price", width: 130 },
  { field: "vehicleFuelFillDate", headerName: "Date", width: 130 },
  { field: "vehicleFuelComments", headerName: "Comments", width: 180 },
];

export default function Overview() {
  const [rows, setRows] = useState([]);
  const [openDialog, setOpenDialog] = useState(false);
  const [selectedVehicleFuelId, setSelectedVehicleFuelId] = useState(null);
  const [selectedFuelData, setSelectedFuelData] = useState(null);

  useEffect(() => {
    fetch("http://localhost:5013/api/Fuel")
      .then((response) => response.json())
      .then((data) => {
        // Map the fetched data to include a unique id for each row
        const rowsWithIds = data.map((row, index) => ({ id: index + 1, ...row }));
        setRows(rowsWithIds);
      })
      .catch((error) => {
        console.error("Error fetching data:", error);
      });
  }, []);

  const handleRowClick = (params) => {
    setSelectedVehicleFuelId(params.row.fuelId);
    setOpenDialog(true);

    fetch(`http://localhost:5013/api/Fuel/${params.row.vehicleFuelId}`)
      .then((response) => response.json())
      .then((data) => {
        setSelectedFuelData(data);
      })
      .catch((error) => {
        console.error("Error fetching selected fuel data:", error);
      });
  };

  const handleCloseDialog = () => {
    setOpenDialog(false);
  };

  return (
    <>
      <Paper sx={{ height: 400, width: "100%" }}>
        <DataGrid
          rows={rows}
          columns={columns}
          pageSize={5}
          disableSelectionOnClick
          onRowClick={handleRowClick}
          sx={{ border: 0 }}
        />
      </Paper>
      <Dialog open={openDialog} onClose={handleCloseDialog} fullWidth maxWidth="md">
        <DialogTitle>
          Fuel Details
          <IconButton
            aria-label="close"
            onClick={handleCloseDialog}
            sx={{ position: "absolute", right: 8, top: 8 }}
          >
            <CloseIcon />
          </IconButton>
        </DialogTitle>
        <DialogContent>{selectedFuelData && <PopFuel fuelData={selectedFuelData} />}</DialogContent>
      </Dialog>
    </>
  );
}
